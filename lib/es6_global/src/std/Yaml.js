...
function classify(value) {
  ...
  switch (match) {
    ...
    default:
      return {
              TAG: /* Object */2,
              _0: Dict.map(value, (function (param) {
                      return [
                              param[0],
                              classify(param[1])
                            ];
...
function _stringify(yml, level) {
  ...
  switch (yml.TAG | 0) {
    case /* String */0 :
        return "\"" + yml._0 + "\"";
    case /* Number */1 :
        return Float.toString(yml._0);
    case /* Object */2 :
        var entries = Dict.toArray(yml._0);
        return (
                level === 0 ? "" : "\n"
              ) + $$Array.join($$Array.mapWithIndex(entries, (function (i, param) {
                          var val = param[1];
                          ...
                          return $$String.repeat("  ", level) + param[0] + ": " + _stringify(val, level + 1 | 0) + (
                                  (i + 1 | 0) === $$Array.length(entries) ? "" : "\n"
                                );
                        })), "");
    case /* Array */3 :
        var array = yml._0;
        return (
                level === 0 ? "" : "\n"
              ) + $$Array.join($$Array.mapWithIndex(array, (function (i, val) {
                          if (typeof val === "number") {
                            return "";
                          } else {
                            return $$String.repeat("  ", level) + "- " + _stringify(val, level + 1 | 0) + (
                                    (i + 1 | 0) === $$Array.length(array) ? "" : "\n"
                                  );
                          }
                        })), "");
    case /* Bool */4 :
