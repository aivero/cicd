...
function findReqs(ints, allInts) {
  return Task.map(Promise.all([
                ...
                return Belt_Result.flatMap(Seq.result2(param[0], param[1]), (function (param) {
                              var ints = param[0];
                              var reqs = Flat.array(Belt_Array.map(ints, (function ($$int) {
                                          var req = $$int.req;
                                          if (req !== undefined) {
                                            return req;
                                          } else {
                              ...
                              var reqs$1 = param[1].filter(function ($$int) {
                                    var name = $$int.name;
                              ...
                              return {
                                      TAG: /* Ok */0,
                                      _0: Belt_Array.concat(ints, reqs$1)
                                    };
                            }));
              }));
}
