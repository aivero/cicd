...
function handleConfigChange(confPath) {
  ...
  return TaskResult.flatMap(TaskResult.seq3([
                ...
                return Task.resolve({
                            TAG: /* Ok */0,
                            _0: param[2].includes(confPath) ? cmpInts(param[0], param[1]) : []
                          });
              }));
}

function handleFileChange(confPath, filePath) {
  return Task.resolve(Belt_Result.map(Config.loadFile(confPath), (function (ints) {
                    return ints.filter(function (param) {
                                return param.folder.endsWith(Path.dirname(filePath));
                              });
                  })));
}
