...
function getInfo(param) {
  ...
  return Task.map(Proc.run(Belt_Array.concatMany([
                ...
                return Belt_Result.flatMap(res, (function (param) {
                              var output = $$File.read(match + "-" + match$1 + "-" + hash + ".json");
                              if (output.TAG !== /* Ok */0) {
                                return {
                                        TAG: /* Error */1,
                                        _0: output._0
...
function conanInit(zips) {
  ...
  var config = Task.sleep(Proc.run([
            "conan",
            "config",
            "install",
            match$2[0],
  ...
  return Task.flatMap(config, (function (config) {
                if (config.TAG === /* Ok */0) {
                  return Task.map(Promise.all(Belt_Array.map(exportPkgs, (function (param) {
                                        return Proc.run([
                                                    "conan",
                                                    "export",
                                                    param[1],
  ...
  return Task.map(Flat.task(Task.map(Flat.task(Task.map(Flat.task(Task.map(pkgInfos, (function (res) {
                                                      ...
                                                      return Task.map(Promise.all(Belt_Array.map(pkgInfos, (function (pkgInfo) {
                                                                            ...
                                                                            if (match !== undefined && match$1 !== undefined) {
                                                                              return Task.map(Proc.run(Belt_Array.concat([
                                                                                                  ...
                                                                                                  "create",
                                                                                                  "--ref=" + match + "/" + match$1,
                                                                                                  "--lockfile-out=" + match + "-" + match$1 + "-" + Hash(pkgInfo) + ".lock",
                                                                                                  "-pr=" + pkgInfo.profile
                                ...
                                return Belt_Result.map(res, (function (pkgInfos) {
                                              var locks = Belt_Array.map(pkgInfos, (function (pkgInfo) {
                                                      ...
                                                      var match$1 = pkgInfo.int.version;
                                                      if (match !== undefined && match$1 !== undefined) {
                                                        return match + "-" + match$1 + "-" + Hash(pkgInfo) + ".lock";
                                                      } else {
                                                        return "";
                                                      }
                                                    }));
  ...
  return Belt_Result.map(Flat.array(Belt_Array.mapWithIndex(buildOrder, (function (index, group) {
                        return Belt_Result.map(Flat.array(Belt_Array.map(group, (function (pkg) {
                                              ...
                                              return Flat.array(Belt_Array.concat(Belt_Array.map(foundPkgs, (function (foundPkg) {
                                                                    ...
                                                                    var match = Belt_Array.get(pkg.split("#"), 0);
                                                                    var match$1 = Detect.getImage({
                                                                          int: $$int,
                                                                          profile: profile,
                                                                          mode: mode
                                                                        });
                                                                    if (match !== undefined && match$1.TAG === /* Ok */0) {
                                                                      var needs = $$int.req;
                                                                      var group = Belt_Array.get(buildOrder, index - 1 | 0);
                                                                      return {
                                                                              TAG: /* Ok */0,
                                                                              _0: {
                                                                                name: match + foundPkg.hash,
                                                                                image: match$1._0,
                                                                                script: getCmds({
                                                                                      int: $$int,
                                                                                      profile: profile,
                                                                ...
                                                                _0: {
                                                                  ...
                                                                  needs: Belt_Array.map(foundPkgs, (function (foundPkg) {
                                                                          var pkg$1 = Belt_Array.get(pkg.split("#"), 0);
                                                                          if (pkg$1 !== undefined) {
                                                                            return pkg$1 + foundPkg.hash;
                                                                          } else {
                                                                            return "Invalid package: " + pkg;
                                                                          }
                                                                        }))
                                                                }
                                                              }]));
...
function getJobs(zips) {
  ...
  var pkgInfos = Task.flatMap(conanInit(zips$1), (function (exportPkgs) {
          if (exportPkgs.TAG === /* Ok */0) {
