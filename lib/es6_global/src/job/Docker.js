  ...
  if (file$1 !== undefined) {
    ...
    return $$Array.map($$Array.filter(Path.read(folder), (function (file) {
                  ...
                  return {
                          ...
                          file: file.name,
                          folder: folder,
                          tags: getTags(getName(file.name, folder)),
                          reqs: reqs
                        };
                }));
  }
...
function getJob(param) {
  ...
  var env = $$Option.seq4([
        ...
        Caml_option.nullable_to_opt(Deno.env.get("DOCKER_REGISTRY")),
        Caml_option.nullable_to_opt(Deno.env.get("DOCKER_PREFIX"))
      ]);
  return Result.map(env !== undefined ? ({
                  TAG: /* Ok */0,
                  _0: env
                }) : ({
                  TAG: /* Error */1,
                  _0: "Docker: Username, password, registry or prefix not provided!"
                }), (function (param) {
...
function getJobs(ints) {
  var ints$1 = $$Array.flatMap(ints, getInstances);
  return Task.resolve(Result.seq($$Array.map(ints$1, getJob)));
}
