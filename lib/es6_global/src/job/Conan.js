...
function init(zips) {
  ...
  return Flat.task(TaskResult.map(config, (function (param) {
                    return Task.map(Task.flatMap(Task.pool(Belt_Array.map(exportPkgs, (function (param, param$1) {
                                              return Proc.run([
                                                          "conan",
                                                          "export",
                                                          param[1],
...
function getLockFile(pkgInfos) {
  return TaskResult.flatMap(Flat.task(TaskResult.map(Flat.task(TaskResult.map(Flat.task(TaskResult.map(pkgInfos, (function (pkgInfos) {
                                        return Task.map(Task.flatMap(Task.pool(Belt_Array.map(pkgInfos, (function (pkgInfo, param) {
...
function getJobs(zips) {
  ...
  var pkgInfos = Flat.task(TaskResult.map(init(zips$1), (function (param) {
              return Task.map(Task.flatMap(Task.pool(Belt_Array.map(zips$1, (function (zip, param) {
                                        return getInfo(zip);
                                      })), 32), (function (prim) {
                                return Promise.all(prim);
                              })), Flat.array);
