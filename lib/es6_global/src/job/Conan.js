...
function getVariables(param) {
  ...
  return Belt_Result.map(getRepo($$int), (function (repo) {
                ...
                var match$3 = match$1.match(/^[0-9a-f]{40}$/);
    ...
    if (match$1 !== undefined) {
      if (match$2 !== undefined) {
        if (repo.TAG === /* Ok */0) {
          ...
          var match$3 = Deno.env.get("CI_COMMIT_REF_NAME");
          var match$4 = match$1.match(/^[0-9a-f]{40}$/);
          var uploadPkgAlias = (match$3 == null) ? [] : (
              match$4 !== null ? [Belt_Array.joinWith([
                        "conan",
                        "upload",
                        match + "/" + match$3 + "@",
                        "--all",
          ...
          var match$5 = $$int.debugPkg;
          var uploadDbg = match$5 !== undefined ? (
              match$5 ? [Belt_Array.joinWith([
                        "conan",
                        "upload",
                        match + "-dbg/" + match$1 + "@",
                        "--all",
...
function init(zips) {
  ...
  return TaskResult.flatten(TaskResult.map(Task.flatMap(config, (function (param) {
                        var match = Deno.env.get("CONAN_LOGIN_USERNAME");
                        var match$1 = Deno.env.get("CONAN_LOGIN_PASSWORD");
                        var match$2 = Deno.env.get("CONAN_REPO_INTERNAL");
...
function getLockFile(pkgInfos) {
  return TaskResult.flatMap(TaskResult.flatten(TaskResult.map(TaskResult.flatten(TaskResult.map(TaskResult.flatten(TaskResult.map(pkgInfos, (function (pkgInfos) {
                                        return TaskResult.pool(Belt_Array.map(pkgInfos, (function (pkgInfo, param) {
                                                          ...
                                                          if (match !== undefined && match$1 !== undefined) {
                                                            return TaskResult.map(Proc.run(Belt_Array.concat([
                                                                                ...
                                                                                "--ref=" + match + "/" + match$1,
                                                                                "--build=" + match + "/" + match$1,
                                                                                "--lockfile-out=" + match + "-" + match$1 + "-" + Hash.hashN(pkgInfo, 3) + ".lock",
                                                                                "-pr=" + pkgInfo.profile
                                ...
                                var locks = Belt_Array.map(pkgInfos, (function (pkgInfo) {
                                        ...
                                        var match$1 = pkgInfo.int.version;
                                        if (match !== undefined && match$1 !== undefined) {
                                          return match + "-" + match$1 + "-" + Hash.hashN(pkgInfo, 3) + ".lock";
                                        } else {
                                          return "";
                                        }
                                      }));
...
function getJob(buildOrder, pkgInfos) {
  return Belt_Result.map(Seq.array(Belt_Array.mapWithIndex(buildOrder, (function (index, group) {
                        return Belt_Result.map(Seq.array(Belt_Array.map(group, (function (pkg) {
                                              var match = pkg.split("#");
                                              if (match.length !== 2) {
                                                throw {
                                              ...
                                              return Seq.array(Belt_Array.concat(Belt_Array.map(foundPkgs, (function (foundPkg) {
