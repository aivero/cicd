// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Fs from "fs";
import * as Env from "../std/Env.js";
import * as Int from "../std/Int.js";
import * as Bool from "../std/Bool.js";
import * as Proc from "../std/Proc.js";
import * as Task from "../std/Task.js";
import Hash from "hash";
import * as Path from "path";
import * as $$Array from "../std/Array.js";
import * as Tuple from "../std/Tuple.js";
import * as Config from "../Config.js";
import * as $$Option from "../std/Option.js";
import * as Result from "../std/Result.js";
import * as $$String from "../std/String.js";
import * as Console from "../std/Console.js";
import * as Instance from "../Instance.js";

function getCurBranch(param) {
  return Env.getError("CI_COMMIT_REF_NAME");
}

function getMergeBase(curBranch, branch) {
  return Task.fold(Task.flatMap(Task.map(Proc.run([
                          "git",
                          "merge-base",
                          curBranch,
                          branch
                        ]), $$String.trim), (function (mergeBase) {
                    return Task.map(Task.flatMap(Proc.run([
                                        "git",
                                        "rev-list",
                                        "--count",
                                        curBranch + "..." + mergeBase
                                      ]), (function (output) {
                                      return Task.fromResult($$Option.toResult(Int.fromString($$String.trim(output)), "Couldn't convert to int"));
                                    })), (function (countMergeBase) {
                                  return [
                                          countMergeBase,
                                          branch,
                                          mergeBase
                                        ];
                                }));
                  })), (function (output) {
                Console.log("Branch \"" + branch + "\" failed with output: \n" + output);
                return [
                        99999,
                        "00000000000",
                        branch
                      ];
              }));
}

function getParentBranch(param) {
  var curBranch = Task.fromResult(Env.getError("CI_COMMIT_REF_NAME"));
  return Task.map(Task.flatMap(Task.seq2([
                      curBranch,
                      Proc.run([
                            "git",
                            "branch",
                            "-a"
                          ])
                    ]), (function (param) {
                    var curBranch = param[0];
                    var branches = $$Array.filter($$Array.map($$String.split($$String.trim(param[1]), "\n"), $$String.trim), (function (branch) {
                            return !($$String.startsWith(branch, "*") || $$String.endsWith(branch, "/" + curBranch));
                          }));
                    return Task.flatMap(Task.seq($$Array.map(branches, (function (param) {
                                          return getMergeBase(curBranch, param);
                                        }))), (function (mergeBases) {
                                  return Task.fromResult($$Option.toResult($$Array.sort(mergeBases, (function (param, param$1) {
                                                          return param[0] - param$1[0] | 0;
                                                        }))[0], "No merge bases"));
                                }));
                  })), (function (param) {
                return [
                        param[1],
                        param[2]
                      ];
              }));
}

function getLastRev(param) {
  var match = Env.getError("CI_COMMIT_REF_NAME");
  var match$1 = Env.getError("CI_COMMIT_BEFORE_SHA");
  if (match.TAG !== /* Ok */0) {
    return Task.toError("Couldn't find last rev");
  }
  if (match._0 === "master" && match$1.TAG === /* Ok */0) {
    var commit = match$1._0;
    if (commit === "0000000000000000000000000000000000000000") {
      return Task.to([
                  "master",
                  "HEAD^"
                ]);
    } else {
      return Task.to([
                  "master",
                  commit
                ]);
    }
  }
  return Task.map(getParentBranch(undefined), (function (param) {
                return [
                        param[0],
                        param[1]
                      ];
              }));
}

function cmpInts(intsNew, intsOld) {
  var hashsOld = $$Array.map(intsOld, (function (prim) {
          return Hash(prim);
        }));
  return $$Array.filter(intsNew, (function (intNew) {
                return !$$Array.includes(hashsOld, Hash(intNew));
              }));
}

function handleConfigChange(confPath) {
  Console.log("Config changed: " + confPath);
  var intsNew = Task.map(Proc.run([
            "git",
            "show",
            "HEAD:" + confPath
          ]), (function (__x) {
          return Config.load(__x, confPath);
        }));
  var lastRev = getLastRev(undefined);
  var intsOld = Task.flatMap(Task.flatMap(lastRev, (function (param) {
              var lastRev = param[1];
              return Task.seq2([
                          Proc.run([
                                "git",
                                "ls-tree",
                                "-r",
                                lastRev
                              ]),
                          Task.to(lastRev)
                        ]);
            })), (function (param) {
          if ($$String.includes(param[0], "\t" + confPath)) {
            return Task.map(Proc.run([
                            "git",
                            "show",
                            param[1] + ":" + confPath
                          ]), (function (__x) {
                          return Config.load(__x, confPath);
                        }));
          } else {
            return Task.to([]);
          }
        }));
  return Task.flatMap(Task.seq2([
                  intsNew,
                  intsOld
                ]), (function (param) {
                return Task.to(cmpInts(param[0], param[1]));
              }));
}

function handleFileChange(confPath, filePath) {
  Console.log("File changed: " + filePath);
  return Task.fromResult(Result.map(Config.loadFile(confPath), (function (__x) {
                    return $$Array.filter(__x, (function (param) {
                                  var folder = param.folder;
                                  var match = $$String.startsWith(Path.dirname(filePath), folder);
                                  Console.log(folder + ": " + Bool.toString(match));
                                  return match;
                                }));
                  })));
}

function handleChange(file) {
  var conf = Config.find(Path.dirname(file));
  if (conf !== undefined) {
    if (Path.basename(file) === Config.name) {
      return [handleConfigChange(conf)];
    } else {
      return [handleFileChange(conf, file)];
    }
  } else {
    return [];
  }
}

function findInts(param) {
  Console.log("Git Mode: Create instances from changed files in git");
  return Task.flatMap(Task.flatMap(Task.flatMap(Task.flatMap(Task.fromResult(Result.seq2(Tuple.map2([
                                      "CI_COMMIT_REF_NAME",
                                      "CI_COMMIT_SHA"
                                    ], Env.getError))), (function (param) {
                            return Proc.run([
                                        "git",
                                        "checkout",
                                        "-B",
                                        param[0],
                                        param[1]
                                      ]);
                          })), (function (param) {
                        return getLastRev(undefined);
                      })), (function (param) {
                    var commit = param[1];
                    Console.log("Last branch: " + param[0]);
                    Console.log("Last commit: " + commit);
                    return Proc.run([
                                "git",
                                "diff",
                                "--name-only",
                                commit,
                                "HEAD"
                              ]);
                  })), (function (output) {
                return Task.map(Task.seq($$Array.flatMap($$Array.filter($$String.split($$String.trim(output), "\n"), (function (prim) {
                                          return Fs.existsSync(prim);
                                        })), handleChange)), (function (confs) {
                              var ints = $$Array.flatten(confs);
                              return $$Array.reduce(ints, (function (param, $$int) {
                                              var ints = param[1];
                                              var intsHash = param[0];
                                              var newHash = Hash($$int);
                                              if ($$Array.some(intsHash, (function (oldHash) {
                                                        return oldHash === newHash;
                                                      }))) {
                                                return [
                                                        intsHash,
                                                        ints
                                                      ];
                                              } else {
                                                Console.log("Found instance: " + $$int.name + "/" + $$int.version + " (mode: " + Instance.modeToString($$int.mode) + ") (" + newHash + ")");
                                                return [
                                                        $$Array.concat(intsHash, [newHash]),
                                                        $$Array.concat(ints, [$$int])
                                                      ];
                                              }
                                            }), [
                                            [],
                                            []
                                          ])[1];
                            }));
              }));
}

export {
  getCurBranch ,
  getMergeBase ,
  getParentBranch ,
  getLastRev ,
  cmpInts ,
  handleConfigChange ,
  handleFileChange ,
  handleChange ,
  findInts ,
  
}
/* fs Not a pure module */
