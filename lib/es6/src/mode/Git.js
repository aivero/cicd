...
function getMergeBase(curBranch, branch) {
  Console$Std.log("Comparing merge branch: " + branch);
...
function getParentBranch(param) {
  Console$Std.log("Finding parent branch");
  ...
  return Task$Std.map(Task$Std.flatMap(Task$Std.seq2([
                    ...
                    var branches = Array$Std.filter(Array$Std.map(String$Std.split(String$Std.trim(param[1]), "\n"), String$Std.trim), (function (branch) {
                            return !(String$Std.startsWith(branch, "*") || String$Std.endsWith(branch, "/" + curBranch));
                          }));
                    Console$Std.log("Branches: " + Array$Std.join(branches, ", "));
