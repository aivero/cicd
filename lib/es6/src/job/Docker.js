...
function getJob(param) {
  ...
  return Result$Std.flatMap(Result$Std.seq4(Tuple$Std.map4([
                ...
                return Result$Std.map(Profile.getPlatform(profile), (function (platform) {
                              var script$1;
                              if (Array$Std.empty(script)) {
                                var dependencyProxy = Deno.env.get("CI_DEPENDENCY_PROXY_SERVER");
                                var proxyUser = Deno.env.get("CI_DEPENDENCY_PROXY_USER");
                                var proxyPassword = Deno.env.get("CI_DEPENDENCY_PROXY_PASSWORD");
                                var proxy;
                                var exit = 0;
                                if ((dependencyProxy == null) || (proxyUser == null) || (proxyPassword == null)) {
                                  exit = 1;
                                } else {
                                  proxy = ["docker login --username " + username + " --password " + password + " " + dependencyProxy];
                                }
                                if (exit === 1) {
                                  proxy = ["docker login --username " + username + " --password " + password + " " + registry];
                                }
                                script$1 = Array$Std.concat(Array$Std.concat(Array$Std.concat(proxy, [
