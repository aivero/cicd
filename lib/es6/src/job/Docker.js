  ...
  return Result$Std.flatMap(Result$Std.seq4(Tuple$Std.map4([
                ...
                return Result$Std.map(Profile.getPlatform(profile), (function (platform) {
                              ...
                              if (Array$Std.empty(script)) {
                                var dependencyProxy = Deno.env.get("DOCKER_DEPENDENCY_PROXY");
                                var dependencyProxyUser = Deno.env.get("CI_DEPENDENCY_PROXY_USER");
                                var dependencyProxyPassword = Deno.env.get("CI_DEPENDENCY_PROXY_PASSWORD");
                                var proxy = (dependencyProxyUser == null) ? [] : (
                                    (dependencyProxyPassword == null) ? [] : (
                                        (dependencyProxy == null) ? [] : ["docker login --username " + dependencyProxyUser + " --password " + dependencyProxyPassword + " " + dependencyProxy]
                                      )
                                  );
