...
function getVariables(param) {
  ...
  return Dict$Std.fromArray(Array$Std.concat(Array$Std.concat(Array$Std.concat([
                          [
                            "NAME",
                            match.name
                          ],
...
function getExtends(param) {
  ...
  var bootstrap = param[2] ? "-bootstrap" : "";
  var match$1 = String$Std.match(param[1], /^[0-9a-f]{40}$/);
  var alias = match$1 !== undefined ? "-alias" : "";
  return Result$Std.map(arch, (function (arch) {
                return [".conan" + "-" + arch + bootstrap + alias];
              }));
}
