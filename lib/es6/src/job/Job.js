...
function handleDuplicates(jobs) {
  return Array$Std.flatMap(Array$Std.groupBy(jobs, (function (param) {
                ...
                var subJobs = Array$Std.map(group, (function (param) {
                        var job = param[1];
                        return [
                ...
                var needs = Array$Std.map(subJobs, (function (param) {
                        return param[1].needs;
                      }));
                var firstNeeds = Option$Std.flat(needs[0]);
                var allNeedsAreTheSame = Array$Std.reduce(Array$Std.map(needs, (function (needs) {
                            return Caml_obj.caml_equal(needs, firstNeeds);
                ...
                var match = Option$Std.map(firstNeeds, Array$Std.length);
                var allThejobs;
                var exit = 0;
                if (allNeedsAreTheSame && !(match !== undefined && !(match > 2 || match < 0))) {
                  exit = 1;
                } else {
                  allThejobs = subJobs;
                }
                if (exit === 1) {
                  ...
                  allThejobs = Array$Std.concat(Array$Std.map(subJobs, (function (param) {
                              var job = param[1];
                              return [
                                      param[0],
                                      {
                ...
                return Array$Std.concat(allThejobs, [[
                              key,
                              {
                                extends: Jobt.$$default.extends,
                                variables: Jobt.$$default.variables,
                                ...
                                script: ["echo"],
                                after_script: Jobt.$$default.after_script,
                                needs: Array$Std.map(subJobs, (function (param) {
                                        return param[0];
                                      })),
                                services: Jobt.$$default.services,
                                cache: Jobt.$$default.cache,
