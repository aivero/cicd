  ...
  (
    ".conan",
    {
      ...
      script: Some([
        ...
        "time conan config set general.default_profile=$PROFILE",
        "time conan config set storage.path=$CONAN_DATA_PATH",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $CONAN_REPO_ALL",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $CONAN_REPO_INTERNAL",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $CONAN_REPO_PUBLIC",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $CONAN_REPO_DEV_ALL",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $CONAN_REPO_DEV_INTERNAL",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $CONAN_REPO_DEV_PUBLIC",
        "time conan create -u . $NAME/$VERSION@ $ARGS",
        "time conan user $CONAN_LOGIN_USERNAME -p $CONAN_LOGIN_PASSWORD -r $REPO",
        "time conan upload $NAME/$VERSION@ --all -c -r $REPO",
        "[[ -n $UPLOAD_ALIAS ]] && time conan upload $NAME/$CI_COMMIT_REF_NAME@ --all -c -r $REPO || echo",
      ]),
      cache: Some({
