...
let getImage = (profile, registry: option<string>, prefix, entrypoint: option<array<string>>) => {
  let registry =
    registry
  ...
  (registry, prefix, os, arch)
  ->Result.seq4
  ->Result.map(((registry, prefix, os, arch)) => `${registry}${prefix}${os}-${arch}`)
  ->Result.map(res => {name: res, entrypoint: entrypoint})
}
